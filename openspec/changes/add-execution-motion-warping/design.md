...
项目已经通过 `AnimatorRootMotionRelay` 把 Animator delta 交给 Player `CharacterMotor` 或权威 NPC Motor，但当前管线只原样消费 Root Motion，不知道目标锚点。Unity 读回显示 `rig_Execute` 长约 2.7 秒并前进约 0.75 米，`rig_Executed` 长约 3.517 秒并包含约 2.29 米的根位移，两者长度和轨迹不同。Animator Controller 的 Combat/Reaction Layer 尚未接入这两个 clip。
...
- 让处决输入在存在合法目标时优先于普通 Attack，并由 Offline/Server 权威重新验证。
- 建立绑定双方、锚点、开始时间和结果的权威处决会话。
- 固定被处决者世界 Transform，只对处决者的 Root Motion 施加有限、可诊断的平移和旋转 Warp。
- 在双方各自动画结束前阻断角色控制与普通战斗碰撞，同时保留本地镜头旋转。
...
- 不增加处决提示 UI、慢动作、镜头剪辑、特写、处决变体、奖励或音效系统。
...
会话接受前，`Dead` 和资格失效优先于处决。会话接受后，普通 Hit、Guard、Parry、PostureBreak 和输入不能打断双方；只有场景卸载、Actor 销毁、网络断开等生命周期事件可以触发确定性清理。目标在配置结果时刻锁存致死结果，但 `Executed` 表现保持到自身动画结束，随后进入 `Dead`。

未采用复用 `Attack` / `Dead` 状态，因为普通攻击只有单方所有权，`Dead` 又会立即切换表现，无法表达双方绑定、固定目标、Warp 和动画长度差。

### 左键处决优先于普通攻击

本地 Player 的 Attack 脉冲先交给处决候选解析器。解析器优先检查当前锁定目标，否则按水平距离、角度和稳定 ActorId 选择最优候选。只有存在本地合法候选时才发送处决请求并消费本次 Attack；没有候选或 Server 拒绝时，本次输入按普通 Attack 处理，Server 拒绝不得在后续帧补触发旧输入。

处决者只允许从 `Idle`、`Move` 或 `Guard` 发起。目标必须存活、未处于其他处决会话，并仍处于 `Parried` 或 `PostureBroken`。权威端使用当前状态与 Transform 重新计算全部条件，不能信任 Client 提交的距离、角度或锚点。
...
### 用 Actor 级战斗抑制表达 HitBox/HurtBox 关闭

会话开始时取消双方当前攻击实例，并为每个 Actor 取得带会话所有权的战斗抑制令牌。`CombatResolver` 在攻击方不能产生命中或目标不能接收命中时跳过事务；现有 HitBox/HurtBox Collider 可以保持启用，但在玩法语义上退出普通战斗判定。处决致死通过独立权威事务提交，不经过普通 HitBox。

抑制令牌在各自处决动画完成、会话清理或 Actor 销毁时幂等释放。CharacterController、NavMesh、地面、墙体和其他环境碰撞保持启用。目标完成后进入 `Dead`，继续沿用死亡状态本身的不可受击规则。

未采用逐个禁用 Collider，因为角色可能有多个身体部位和武器 Collider，异常退出时容易漏恢复；关闭物理组件也会破坏 Warp 的墙体约束。

### 玩法时钟与动画表现分离

双方从同一权威开始时间播放 `rig_Execute` / `rig_Executed`，但使用独立配置时长。处决者在约 2.7 秒动画结束后退出控制锁定并释放自身战斗抑制；目标保持固定、不可操纵和不可受击到约 3.517 秒动画结束，然后进入 `Dead`。处决会话在双方都完成或被清理后结束。
...
远端通过会话边沿锁存 `Executing` / `Executed`，旧 Attack、Parried 或 PostureBroken 快照不能恢复动作。重复开始、Host 回环、乱序完成和迟到普通动作必须按 `executionId` 去重。加入较晚或表现延迟的观察者采用当前权威会话与绝对 Transform，不从动画当前帧反推玩法状态。Offline 模式走同一服务接口，但直接在本地创建会话。

### 配置与诊断保持数据驱动

战斗/表现数据至少公开最大处决距离、正面半角、最大高度差、视线/路径层、处决者锚点偏移、最大 Warp 平移/Yaw、Warp Window、Warp 曲线、双方动画时长、CrossFade 和结果提交时间。校验必须保持距离和时长非负、角度有限、Warp Window 有序且结果时间位于会话有效范围。
...
- [风险] 关闭原始 Collider 会造成漏恢复或环境穿透。-> 缓解：使用 Actor 级、会话所有权的战斗抑制令牌，保留环境碰撞组件。
- [风险] Client 延迟让左键处决反馈晚于普通攻击。-> 缓解：本地只在发现候选时消费输入并立即播放非承诺提示；第一版不预测权威位移或致死结果。
- [风险] 目标已逻辑死亡但 `Executed` 动画尚未结束，旧快照可能切回 `Dead` clip。-> 缓解：会话锁存的表现优先级覆盖普通 Dead 表现，结束边沿后才交给 `Dead`。
- [权衡] 处决者动画结束早于目标。-> 处决者按自身 2.7 秒解锁，目标继续完成约 3.517 秒表现；会话保留到双方结束，避免额外冻结处决者约 0.817 秒。

## 迁移计划

1. 增加配置、枚举、会话数据与诊断，但保持默认场景未触发处决时行为不变。
2. 先完成 Offline Player 对 NPC 的候选、状态、固定目标、Warp、战斗抑制和致死闭环。
3. 接入 Player 目标与 Server 权威会话协议，验证 Host/Client 去重、乱序和绝对位置。
4. 接入 Animator Controller、Presenter、镜头模式和数据资产，使用真实 clip 校准锚点与时间。
5. 更新项目需求、路线与验证矩阵，并在取得运行证据前保持网络项未验证。
